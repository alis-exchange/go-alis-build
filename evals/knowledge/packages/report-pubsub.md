...
Default topic: `alis.evals.v1.Run`. **This topic is not auto-provisioned**
by the Alis Build platform (not an `Event`-suffixed message). Provision
the topic and subscription via Terraform.
...
| Option | Effect |
| ------ | ------ |
| `WithProject(id)` | Override project (defaults to `ALIS_OS_PROJECT`). Only with `New`. |
...
BigQuery subscription in **table-schema** mode.

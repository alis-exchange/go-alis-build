...
On Alis Build products, evals reporting infrastructure is provisioned
automatically in the **product** GCP project (`ALIS_OS_PRODUCT_PROJECT`):

- Pub/Sub topic `alis.evals.v1.Run`
- BigQuery dataset `evals`, table `runs` (schema from `bqschema.SchemaJSON()`)
- Pub/Sub → BigQuery subscription in table-schema mode

Environment Terraform grants the env `alis-build` service account
`pubsub.publisher` on the product topic. Neurons run in the environment
project (`ALIS_OS_PROJECT`) but publish eval results to the product project.
`Run.google_project_id` still records `ALIS_OS_PROJECT` (where the eval ran).

The recommended write path from environment neurons is Pub/Sub only
(`pubsubreport.New(ctx)` — no extra args when `ALIS_OS_PRODUCT_PROJECT` is
set). Direct BigQuery streaming inserts require `roles/bigquery.dataEditor`
on the product dataset, which is not granted to environment service accounts
by default.

#### BigQuery reporter

The BigQuery reporter writes each completed `Run` to a pre-existing table whose schema matches `bqschema.Schema()` (equivalently `bqreport.InferSchema()`, which delegates to `bqschema`). Pass the **product** GCP project ID (`ALIS_OS_PRODUCT_PROJECT` on Alis Build), not the environment project where the neuron runs. Schema derivation is now owned by the companion `bqschema` package so that both this reporter and Pub/Sub → BigQuery subscriptions land rows in the same layout. Type mapping:
...
// bq mk --table ALIS_OS_PRODUCT_PROJECT:evals.runs schema.json
```
...
The default topic is `"alis.evals.v1.Run"` (the payload's proto full name). On Alis Build products this topic (and its Pub/Sub → BigQuery subscription) is provisioned in the product GCP project via Terraform; unlike `*Event`-suffixed messages it is not created by the define step. Override with `WithTopic(...)` when your platform provisions under a different name.

`New` resolves the Google Cloud project from `WithProject` when set, otherwise from `ALIS_OS_PRODUCT_PROJECT` (the product project where the topic lives). Do not use `ALIS_OS_PROJECT` — that is the environment project where the neuron runs.
